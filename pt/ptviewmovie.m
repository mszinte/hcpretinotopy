function [timeframes,timekeys,digitrecord,trialoffsets] = ...
  ptviewmovie(vid,images,frameorder,framecolor,frameduration,fixationorder,fixationcolor,fixationsize, ...
...
%      sound(sin(1:2000),100);
      ...
      % make video
      vid.vid_num = vid.vid_num + 1;
      image_vid = Screen('GetImage', win);
      image_crop = imcrop(image_vid,vid.crop_rect);
      image_crop_resize = imresize(image_crop,[vid.res(2),vid.res(1)]);
      if vid.make_png
          imwrite(image_crop_resize ,sprintf('%s_frame_%i.png',vid.vid_image_fn,vid.vid_num))
      end
      writeVideo(vid.vid_obj,image_crop_resize);
